...
function main( array $argv ): int {
	...
	foreach ( $hooks_list as $hook ) {
		...
		$index_entries[ $segment . '/' . $filename ] = array(
			'hook_id' => $hook['name'],
			'type'    => $hook['type'],
		);
	}

	// Write index.php manifest.
	$index_content = "<?php\n// Generated by scan-hooks.php — do not edit manually.\n";
	$index_content .= "// Updated: " . gmdate( 'c' ) . "\n";
	$index_content .= 'return ' . var_export( array(
	...
	fwrite( STDERR, sprintf(
		"\nWrote %d hook files to %s (%d actions, %d filters, %d undocumented)\n",
		$total, $docs_dir, $action_count, $filter_count, $undocumented
	) );
...
function infer_plugin( string $filepath ): string {
	if ( str_contains( $filepath, 'uncanny-automator-pro' ) ) {
		return 'uncanny-automator-pro';
	}
...
function infer_segment( string $filepath ): string {
	$is_pro = str_contains( $filepath, 'uncanny-automator-pro' );
	$prefix = $is_pro ? 'pro-' : '';

	if ( preg_match( '#/integrations/([^/]+)/#', $filepath, $m ) ) {
		return $prefix . 'integrations/' . $m[1];
	}
